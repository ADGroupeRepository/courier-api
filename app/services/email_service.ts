...
export default class EmailService {
  ...
  static async send(payload: EmailPayload): Promise<void> {
    try {
      ...
      logger.info({ id: data?.id, to: payload.to, subject: payload.subject }, 'Email sent via Resend')
    } catch (err: any) {
      logger.error({ err, to: payload.to, subject: payload.subject }, 'Unexpected error sending email via Resend')
    }
  }
}
