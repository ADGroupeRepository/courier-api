...
export default class ModuleGuardMiddleware {
  async handle(ctx: HttpContext, next: NextFn, moduleName: string) {
    ...
    if (!MODULE_REGISTRY.has(moduleName)) {
      return ctx.response.internalServerError({ message: `Module "${moduleName}" is not registered in the system` })
    }
    ...
    try {
      ...
      return ctx.response.internalServerError({ message: 'Error verifying module access', error: error.message })
    }
