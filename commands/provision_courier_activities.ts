...
export default class ProvisionCourierActivities extends BaseCommand {
  static readonly commandName = 'provision:courier-activities'
  static readonly description =
    'Creates the courier_activities collection for organisations that already have the courier module active'
  ...
  async run() {
    ...
    try {
      ...
      this.logger.success(
        `Courier activities provisioning completed. Provisioned: ${provisionedCount}, skipped: ${skippedCount}, failed: ${failedCount}.`
      )
      ...
      this.logger.error(`Provisioning courier activities failed: ${error.message}`)
      this.exitCode = 1
    }
  }
}
